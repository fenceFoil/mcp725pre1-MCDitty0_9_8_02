...
public class GuiScreenTuneNoteblock extends GuiScreen {

	private static final String TRANSPOSE_SLIDER_LABEL = "Transpose Keys";

	private Point3D noteblockPoint;
	private TileEntityNoteMCDitty tile = new TileEntityNoteMCDitty();
	private static GuiMCDittySlider transpostionSlider;
	...
	public GuiScreenTuneNoteblock(Point3D hoverPoint) {
		super();
		...
		if (unknownEntity instanceof TileEntityNoteMCDitty) {
			tile = (TileEntityNoteMCDitty) unknownEntity;
		}
	}

	@Override
	public void initGui() {
		// Add MCDitty version readout
		controlList.add(new MCDittyVersionReadoutGuiElement(0));
					...
					if (!tile.noteValueKnown) {
						...
						if (clicksToGo == Integer.MIN_VALUE) {
							...
							if (clicksToGo == 0) {
								...
								BlockNoteMCDitty.chimeBlockAtPitch(tile,
