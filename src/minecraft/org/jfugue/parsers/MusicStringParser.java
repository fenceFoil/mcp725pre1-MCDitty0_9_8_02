...
//        dictionaryMap = dict;
    ...
    private void parseToken(String s) throws JFugueException
    {
        ...
        switch(s.charAt(0))
        {
            ...
            case 'Y' : parseLyricElement(s); break; // After MCDitty
            case '~' : parseMCDittyElement(s); break; // MCDitty
    ...
    public void parseTokenStrict(String s) throws JFugueException
    {
        ...
        switch(s.charAt(0))
        {
            ...
            case 'Y' : parseLyricElement(s); break; // After MCDitty
            case '~' : parseMCDittyElement(s); break; // MCDitty
            default  : new JFugueException("Not a MusicString token: ", s);
        }
    }
	...
	byte voiceNumber;
        ...
        if ((voiceNumber < 0) || (voiceNumber > 15)) {
            throw new JFugueException(JFugueException.VOICE_EXC, Byte.toString(voiceNumber), "V"+s);
        }
    ...
    private void parseTempoElement(String s) throws JFugueException
    {
		...
		if (s.substring(1,s.length()).matches("\\d*")) {
			try {
				tempoForRangeCheck = Integer.parseInt(s.substring(1,s.length()));
			} catch (NumberFormatException e) {
				e.printStackTrace();
			}
		}
		if (tempoForRangeCheck < 20) {
			throw new JFugueException(JFugueException.TEMPO_LOW_RANGE_EXCEPTION, tempoForRangeCheck);
		} else if (tempoForRangeCheck > 300) {
			throw new JFugueException(JFugueException.TEMPO_HIGH_RANGE_EXCEPTION, tempoForRangeCheck);
		}
